...
function hookupPlotlyRelayout(domName, jsFunction) {
  var graph = document.getElementById(domName);
  graph.on('plotly_relayout', jsFunction);
}


function hookupPlotlyEvent(domName, hookupEventName, jsFunction){
  var graph = document.getElementById(domName);
  graph.on(hookupEventName, jsFunction);
}

function unHookPlotlyRelayout(domName, jsFunction) {
  var graph = document.getElementById(domName);
  graph.removeEventListener('plotly_relayout', jsFunction);
}
